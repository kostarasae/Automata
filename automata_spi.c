...
{
	if (pointer == NULL) return SPI_RETURN_T::SPI_RETURN_nullptr;
...
{
	if (var < min) return SPI_RETURN_T::SPI_RETURN_invalid_arg;
	if (var > max) return SPI_RETURN_T::SPI_RETURN_invalid_arg;
	return  SPI_RETURN_T::SPI_RETURN_ok;
}
...
SPI_RETURN_T SPI_configureTransfer(const SPI_CONFIG_T *config)
{
	...
	if (SPI_checkNullPointer(config)) return SPI_RETURN_T::SPI_RETURN_nullptr;

	if (SPI_checkRange(SPI_POLARITY_T::SPI_ACTIVE_LOW, SPI_POLARITY_T::SPI_ACTIVE_HIGH, config.cs_pol) ||
...
SPI_RETURN_T SPI_transferBlocking(const SPI_TRANSFER_T *transfer)
{
	...
	if (SPI_checkRange(MIN_BYTES, MAX_BYTES, transfer.number_of_bytes) return SPI_RETURN_T::SPI_RETURN_invalid_arg;
